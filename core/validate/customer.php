<?php
    ...
    if(isset($_POST['btnEditAccount']) && !empty($_POST['btnEditAccount'])){
        ...
        $getUser = $admin->fetchSingle('tblcustomers','id',$user_id);

        // Form Validation 
        if(empty($fullname) || empty($email) || empty($gender) || empty($phone) || empty($address)){
            ...
            if($cus->editCustomer($user_id,$fullname,$email,$phone,$gender,$address) === true){
        ...
        if(empty($old_password) || empty($new_password) || empty($c_password)){
            ...
            if(password_verify($old_password, $getUser->password)){ # checking if password match
                ...
                if($cus->changeCustomerPassword($getUser->id,$newpassword) === true){
        ...
        $target = 'assets/images/customer_image/' . $image_name;
