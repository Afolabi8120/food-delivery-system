<?php
	...
	class Product extends Admin {
		...
		public function fetchAllProduct(){
			$stmt = $this->pdo->prepare("SELECT * FROM tblproduct WHERE status = 1 AND quantity > 0 ORDER BY RAND() ");
			$stmt->execute();

			return $stmt->fetchAll(PDO::FETCH_OBJ);
		}

		public function fetchAllProductByCategory($category_name){
			$stmt = $this->pdo->prepare("SELECT * FROM tblproduct AS pro INNER JOIN tblcategory AS cat ON cat.cat_id = pro.category_id WHERE cat.name = :category_name ");
		...
		// add product
		public function addProduct($product_code,$product_name,$category_id,$old_price,$new_price,$quantity,$unit,$product_image,$reorder_level,$status,$description){
			$stmt = $this->pdo->prepare("INSERT INTO tblproduct (product_code,product_name,category_id,old_price,new_price,quantity,unit,product_image,reorder_level,status,description) VALUES(:product_code,:product_name,:barcode,:supplier_id,:category_id,:old_price,:new_price,:quantity,:unit,:product_image,:reorder_level,:status,:description)");
			$stmt->bindParam(":product_code", $product_code, PDO::PARAM_STR);
			$stmt->bindParam(":product_name", $product_name, PDO::PARAM_STR);
		...
		// edit product
		public function editProduct($product_id,$product_name,$category_id,$old_price,$new_price,$quantity,$unit,$product_image,$reorder_level,$status,$description){
			$stmt = $this->pdo->prepare("UPDATE tblproduct SET product_name=:product_name,category_id=:category_id,old_price=:old_price,new_price=:new_price,quantity=:quantity,unit=:unit,product_image=:product_image,reorder_level=:reorder_level,status=:status,description=:description WHERE product_id=:product_id ");
		...
		public function getItemsTotal(){
        	$stmt = $this->pdo->prepare("SELECT SUM(new_price * quantity) FROM `tblproduct` ");
			$stmt->execute();
